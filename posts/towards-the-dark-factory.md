...
The type of work we hand to clankers isn't really limited by size. A clanker can take on quite complex tasks, as long as the product outcome is clear (a clanker shouldn't be making product decisions) and the design is either obvious, not important, or close enough to something that already exists for the clanker to pattern-match against. Beyond that you hit diminishing returns: working out a tricky design through issue comments is painful, and much easier in an interactive session. Once the design is settled I attach it to the issue and hand the implementation to a clanker, rather than driving it myself in Claude Code.
